...
class ApacheLuceneSearchComponentImpl extends AbstractSearchComponentImpl {
    ...
    ApacheLuceneSearchComponentImpl(File dataDirectory) {
        this.indexDirectory = new File(dataDirectory, "index");
    }
    ...
    public void index(Workspace workspace) {
        try {
            ...
            doc.add(new TextField(WORKSPACE_KEY, makeWorkspaceIdUnique(workspace.getId()), Field.Store.YES));
            ...
            for (SoftwareSystem softwareSystem : workspace.getModel().getSoftwareSystems()) {
                ...
                for (Container container : softwareSystem.getContainers()) {
                    index(workspace, container, container.getDocumentation(), writer);
                }
            }
    ...
    private void delete(long workspaceId, IndexWriter indexWriter) {
        try {
            Term workspaceIdTerm = new Term(WORKSPACE_KEY, makeWorkspaceIdUnique(workspaceId));
    ...
    private void index(Workspace workspace, Element element, Documentation documentation, IndexWriter indexWriter) throws Exception {
        if (documentation != null) {
            for (Section section : documentation.getSections()) {
                index(workspace, element, section, indexWriter);
            }

            for (Decision decision : documentation.getDecisions()) {
                index(workspace, element, decision, indexWriter);
            }
        }
    }
    ...
    public List<SearchResult> search(String query, String type, Set<Long> workspaceIds) {
        ...
        try {
            ...
            for (Long workspaceId : workspaceIds) {
                workspaceQueryBuilder.add(new TermQuery(new Term(WORKSPACE_KEY, makeWorkspaceIdUnique(workspaceId))), BooleanClause.Occur.SHOULD);
            }
